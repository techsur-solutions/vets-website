...
if [ "$(find src -name '*.cypress.spec.js' | wc -l)" -eq 0 ]; then
  ...
  yarn start-server-and-test "node src/platform/testing/e2e/test-server.js --buildtype=vagovprod --port=3001 > /dev/null 2>&1" :3001 "yarn cy:run --headless --browser firefox $reporterArgs"
fi
